...
class Note:
    ...
    def from_dict(data):
        ...
        note.created_at = datetime.strptime(data.get("created_at", datetime.now().strftime("%d.%m.%Y %H:%M:%S")), "%d.%m.%Y %H:%M:%S")
        return note
    ...
    def __str__(self):
        ...
        """
        due_date_str = self.due_date.strftime("%d.%m.%Y") if self.due_date else "Not specified"
        return (f"Title: {self.title}\nContent: {self.content}\nTags: {', '.join(self.tags)}"
                f"\nDue Date: {due_date_str}\nCreated At: {self.created_at.strftime('%d.%m.%Y %H:%M:%S')}")
